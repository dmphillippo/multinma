...
For purposes of plotting only, we apply a continuity correction to the Abe 2006 study, in which no events were observed on placebo.
...
## Further results
For comparison with @TSD3, we can produce relative effects against placebo at the average observed log odds, using the `relative_effects()` function:
...
Without providing `newdata`, `relative_effects()` will produce relative effects per study.
...
The estimated study-specific intercepts are used in the baseline risk meta-regression.
