...
Again, we use $\mathrm{N}(0, 10^2)$ prior distributions for the treatment effects $d_k$ and study-specific intercepts $\mu_j$, and we additionally use a $\mathrm{half-N}(2.5^2)$ prior for the heterogeneity standard deviation $\tau$.
We can examine the range of parameter values implied by these prior distributions with the `summary()` method:
```{r}
summary(normal(scale = 10))
...
To diagnose, we use the `pairs()` method to investigate where in the posterior distribution these divergences are happening (indicated by red dots):
```{r plot_arm_RE_pairs, fig.width = 6}
pairs(arm_fit_RE, pars = c("mu[4]", "d[3]", "delta[8]", "tau"))
```
...
#### Fixed effect meta-analysis
First, we fit a fixed effect model using the `nma()` function with `trt_effects = "fixed"`.
We use $\mathrm{N}(0, 10^2)$ prior distributions for the treatment effects $d_k$.
We can examine the range of parameter values implied by these prior distributions with the `summary()` method:
```{r}
summary(normal(scale = 10))
```
...
#### Random effects meta-analysis
We now fit a random effects model using the `nma()` function with `trt_effects = "random"`.
Again, we use $\mathrm{N}(0, 10^2)$ prior distributions for the treatment effects $d_k$, and we additionally use a $\mathrm{half-N}(2.5^2)$ prior for the heterogeneity standard deviation $\tau$.
We can examine the range of parameter values implied by these prior distributions with the `summary()` method:
```{r}
summary(normal(scale = 10))
summary(half_normal(scale = 2.5))
...
pairs(contr_fit_RE, pars = c("d[3]", "delta[5]", "tau"))
```
...
pairs(mix_fit_RE, pars = c("d[3]", "delta[8]", "tau"))
```
...
```{r parkinsons_tests, include=FALSE, eval=testthat::is_testing()}
...
test_that("FE relative effects", {
  expect_equal(arm_releff_FE$mean, tsd_FE$mean, tolerance = tol)
  expect_equal(arm_releff_FE$sd, tsd_FE$sd, tolerance = tol)
  expect_equal(arm_releff_FE$`50%`, tsd_FE$median, tolerance = tol)
  expect_equal(arm_releff_FE$`2.5%`, tsd_FE$lower, tolerance = tol)
  expect_equal(arm_releff_FE$`97.5%`, tsd_FE$upper, tolerance = tol)
})
...
test_that("RE relative effects", {
  expect_equal(arm_releff_RE$mean, tsd_RE$mean, tolerance = tol)
  expect_equal(arm_releff_RE$sd, tsd_RE$sd, tolerance = tol)
  expect_equal(arm_releff_RE$`50%`, tsd_RE$median, tolerance = tol)
  expect_equal(arm_releff_RE$`2.5%`, tsd_RE$lower, tolerance = tol)
  expect_equal(arm_releff_RE$`97.5%`, tsd_RE$upper, tolerance = tol)
})
...
test_that("RE heterogeneity SD", {
  expect_equal(arm_tau$mean, 0.4, tolerance = tol)
  expect_equal(arm_tau$sd, 0.43, tolerance = tol)
  expect_equal(arm_tau$`50%`, 0.28, tolerance = tol)
  expect_equal(arm_tau$`2.5%`, 0.01, tolerance = tol)
  expect_equal(arm_tau$`97.5%`, 1.55, tolerance = tol)
})
...
test_that("FE predicted probabilities at 3 years", {
  expect_equal(arm_pred_FE$mean, tsd_pred_FE$mean, tolerance = tol)
  expect_equal(arm_pred_FE$sd, tsd_pred_FE$sd, tolerance = tol)
  expect_equal(arm_pred_FE$`50%`, tsd_pred_FE$median, tolerance = tol)
  expect_equal(arm_pred_FE$`2.5%`, tsd_pred_FE$lower, tolerance = tol)
  expect_equal(arm_pred_FE$`97.5%`, tsd_pred_FE$upper, tolerance = tol)
})
...
test_that("RE predicted probabilities at 3 years", {
  expect_equal(arm_pred_RE$mean, tsd_pred_RE$mean, tolerance = tol)
  expect_equal(arm_pred_RE$sd, tsd_pred_RE$sd, tolerance = tol)
  expect_equal(arm_pred_RE$`50%`, tsd_pred_RE$median, tolerance = tol)
  expect_equal(arm_pred_RE$`2.5%`, tsd_pred_RE$lower, tolerance = tol)
  expect_equal(arm_pred_RE$`97.5%`, tsd_pred_RE$upper, tolerance = tol)
})

# FE DIC
test_that("FE DIC", {
  expect_equal(arm_dic_FE$resdev, 13.3, tolerance = tol_dic)
  expect_equal(arm_dic_FE$pd, 11.0, tolerance = tol_dic)
  expect_equal(arm_dic_FE$dic, 24.3, tolerance = tol_dic)
})

# RE DIC
test_that("RE DIC", {
  expect_equal(arm_dic_RE$resdev, 13.6, tolerance = tol_dic)
  expect_equal(arm_dic_RE$pd, 12.4, tolerance = tol_dic)
  expect_equal(arm_dic_RE$dic, 26.0, tolerance = tol_dic)
})
...
test_that("FE relative effects", {
  expect_equal(contr_releff_FE$mean, tsd_FE$mean, tolerance = tol)
  expect_equal(contr_releff_FE$sd, tsd_FE$sd, tolerance = tol)
  expect_equal(contr_releff_FE$`50%`, tsd_FE$median, tolerance = tol)
  expect_equal(contr_releff_FE$`2.5%`, tsd_FE$lower, tolerance = tol)
  expect_equal(contr_releff_FE$`97.5%`, tsd_FE$upper, tolerance = tol)
})

contr_releff_RE <- as.data.frame(contr_releff_RE)

test_that("RE relative effects", {
  expect_equal(contr_releff_RE$mean, tsd_RE$mean, tolerance = tol)
  expect_equal(contr_releff_RE$sd, tsd_RE$sd, tolerance = tol)
  expect_equal(contr_releff_RE$`50%`, tsd_RE$median, tolerance = tol)
  expect_equal(contr_releff_RE$`2.5%`, tsd_RE$lower, tolerance = tol)
  expect_equal(contr_releff_RE$`97.5%`, tsd_RE$upper, tolerance = tol)
})
...
test_that("RE heterogeneity SD", {
  expect_equal(contr_tau$mean, 0.4, tolerance = tol)
  expect_equal(contr_tau$sd, 0.43, tolerance = tol)
  expect_equal(contr_tau$`50%`, 0.28, tolerance = tol)
  expect_equal(contr_tau$`2.5%`, 0.01, tolerance = tol)
  expect_equal(contr_tau$`97.5%`, 1.55, tolerance = tol)
})

contr_pred_FE <- as.data.frame(contr_pred_FE)

test_that("FE predicted probabilities at 3 years", {
  expect_equal(contr_pred_FE$mean, tsd_pred_FE$mean, tolerance = tol)
  expect_equal(contr_pred_FE$sd, tsd_pred_FE$sd, tolerance = tol)
  expect_equal(contr_pred_FE$`50%`, tsd_pred_FE$median, tolerance = tol)
  expect_equal(contr_pred_FE$`2.5%`, tsd_pred_FE$lower, tolerance = tol)
  expect_equal(contr_pred_FE$`97.5%`, tsd_pred_FE$upper, tolerance = tol)
})

contr_pred_RE <- as.data.frame(contr_pred_RE)

test_that("RE predicted probabilities at 3 years", {
  expect_equal(contr_pred_RE$mean, tsd_pred_RE$mean, tolerance = tol)
  expect_equal(contr_pred_RE$sd, tsd_pred_RE$sd, tolerance = tol)
  expect_equal(contr_pred_RE$`50%`, tsd_pred_RE$median, tolerance = tol)
  expect_equal(contr_pred_RE$`2.5%`, tsd_pred_RE$lower, tolerance = tol)
  expect_equal(contr_pred_RE$`97.5%`, tsd_pred_RE$upper, tolerance = tol)
})

# FE DIC
test_that("FE DIC", {
  expect_equal(contr_dic_FE$resdev, 6.3, tolerance = tol_dic)
  expect_equal(contr_dic_FE$pd, 4.0, tolerance = tol_dic)
  expect_equal(contr_dic_FE$dic, 10.3, tolerance = tol_dic)
})

# RE DIC
test_that("RE DIC", {
  expect_equal(contr_dic_RE$resdev, 6.6, tolerance = tol_dic)
  expect_equal(contr_dic_RE$pd, 5.5, tolerance = tol_dic)
  expect_equal(contr_dic_RE$dic, 12.1, tolerance = tol_dic)
})
...
expect_equal_nma_summary <- function(object, expected, ...) {
  
  act <- quasi_label(enquo(object), arg = "object")
  exp <- quasi_label(enquo(expected), arg = "expected")
  
  comp <- compare(act$val %>% as.data.frame() %>% select(-c(se_mean, Bulk_ESS, Tail_ESS, Rhat)), 
                  exp$val %>% as.data.frame() %>% select(-c(se_mean, Bulk_ESS, Tail_ESS, Rhat)), 
                  ...)
...
test_that("Analysis of reordered contrast data gives same results", {
  expect_equal_nma_summary(relative_effects(reorder_fit_FE, trt_ref = 1),
                           contr_releff_FE, 
                           tolerance = tol)
  expect_equal_nma_summary(relative_effects(reorder_fit_RE, trt_ref = 1),
                           contr_releff_RE, 
                           tolerance = tol)
  
  expect_equal_nma_summary(predict(reorder_fit_FE, 
                                     baseline = distr(qnorm, mean = -0.73, sd = 21^-0.5),
                                     type = "response",
                                     trt_ref = 1),
                           contr_pred_FE,
                           tolerance = tol)
  expect_equal_nma_summary(predict(reorder_fit_RE, 
                                     baseline = distr(qnorm, mean = -0.73, sd = 21^-0.5),
                                     type = "response",
                                     trt_ref = 1),
                           contr_pred_RE,
  ...
  expect_equal(dic(reorder_fit_FE)$resdev, contr_dic_FE$resdev, tolerance = tol_dic)
  expect_equal(dic(reorder_fit_FE)$pd, contr_dic_FE$pd, tolerance = tol_dic)
  expect_equal(dic(reorder_fit_FE)$dic, contr_dic_FE$dic, tolerance = tol_dic)
  expect_equal(dic(reorder_fit_RE)$resdev, contr_dic_RE$resdev, tolerance = tol_dic)
  expect_equal(dic(reorder_fit_RE)$pd, contr_dic_RE$pd, tolerance = tol_dic)
  expect_equal(dic(reorder_fit_RE)$dic, contr_dic_RE$dic, tolerance = tol_dic)
  
  expect_equal_nma_summary(summary(reorder_fit_RE, pars = "tau"), 
                           contr_tau,
                           tolerance = tol)
})
...
test_that("FE relative effects", {
  expect_equal(mix_releff_FE$mean, tsd_FE$mean, tolerance = tol)
  expect_equal(mix_releff_FE$sd, tsd_FE$sd, tolerance = tol)
  expect_equal(mix_releff_FE$`50%`, tsd_FE$median, tolerance = tol)
  expect_equal(mix_releff_FE$`2.5%`, tsd_FE$lower, tolerance = tol)
  expect_equal(mix_releff_FE$`97.5%`, tsd_FE$upper, tolerance = tol)
})

mix_releff_RE <- as.data.frame(mix_releff_RE)

test_that("RE relative effects", {
  expect_equal(mix_releff_RE$mean, tsd_RE$mean, tolerance = tol)
  expect_equal(mix_releff_RE$sd, tsd_RE$sd, tolerance = tol)
  expect_equal(mix_releff_RE$`50%`, tsd_RE$median, tolerance = tol)
  expect_equal(mix_releff_RE$`2.5%`, tsd_RE$lower, tolerance = tol)
  expect_equal(mix_releff_RE$`97.5%`, tsd_RE$upper, tolerance = tol)
})
...
test_that("RE heterogeneity SD", {
  expect_equal(mix_tau$mean, 0.4, tolerance = tol)
  expect_equal(mix_tau$sd, 0.43, tolerance = tol)
  expect_equal(mix_tau$`50%`, 0.28, tolerance = tol)
  expect_equal(mix_tau$`2.5%`, 0.01, tolerance = tol)
  expect_equal(mix_tau$`97.5%`, 1.55, tolerance = tol)
})

mix_pred_FE <- as.data.frame(mix_pred_FE)

test_that("FE predicted probabilities at 3 years", {
  expect_equal(mix_pred_FE$mean, tsd_pred_FE$mean, tolerance = tol)
  expect_equal(mix_pred_FE$sd, tsd_pred_FE$sd, tolerance = tol)
  expect_equal(mix_pred_FE$`50%`, tsd_pred_FE$median, tolerance = tol)
  expect_equal(mix_pred_FE$`2.5%`, tsd_pred_FE$lower, tolerance = tol)
  expect_equal(mix_pred_FE$`97.5%`, tsd_pred_FE$upper, tolerance = tol)
})

mix_pred_RE <- as.data.frame(mix_pred_RE)

test_that("RE predicted probabilities at 3 years", {
  expect_equal(mix_pred_RE$mean, tsd_pred_RE$mean, tolerance = tol)
  expect_equal(mix_pred_RE$sd, tsd_pred_RE$sd, tolerance = tol)
  expect_equal(mix_pred_RE$`50%`, tsd_pred_RE$median, tolerance = tol)
  expect_equal(mix_pred_RE$`2.5%`, tsd_pred_RE$lower, tolerance = tol)
  expect_equal(mix_pred_RE$`97.5%`, tsd_pred_RE$upper, tolerance = tol)
})

# FE DIC
test_that("FE DIC", {
  expect_equal(mix_dic_FE$resdev, 9.3, tolerance = tol_dic)
  expect_equal(mix_dic_FE$pd, 7.0, tolerance = tol_dic)
  expect_equal(mix_dic_FE$dic, 16.3, tolerance = tol_dic)
})

# RE DIC
test_that("RE DIC", {
  expect_equal(mix_dic_RE$resdev, 9.6, tolerance = tol_dic)
  expect_equal(mix_dic_RE$pd, 8.5, tolerance = tol_dic)
  expect_equal(mix_dic_RE$dic, 18.1, tolerance = tol_dic)
})
```
