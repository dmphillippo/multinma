...
if (class_effects) {
  for (t in 1:(nt-1)) {  // for every treatment
    if (CE_vector_num[t] != 0) {   // if there is more then 1 of this treatment class then it wont be zero
      d[t] ~ normal(trt_class_mean[CE_vector_num[t]], trt_class_sd[CEsd_vector_num[t]]); // apply the trt_class_mean on that treatment
    } else {
