...
We fit a proportional hazards survival model with cubic M-splines on the baseline hazard [@Phillippo_mspline,@Phillippo_survival].
...
Overfitting is avoided by the use of a random walk prior distribution on the (inverse softmax transformed) spline coefficients that penalises complexity and shrinks towards a constant baseline hazard [@Phillippo_mspline]; in practice this means that the number of knots can be set to a sufficiently large number and left to shrink to a suitable level of complexity controlled by the standard deviation of the random walk.
...
We can relax and assess the proportional hazards (PH) assumption by allowing the spline coefficients to vary between treatment arms within each study [@Phillippo_mspline].
