...
Following [@methods_paper] we will use a probit link function (`link = "probit"`), and specify weakly-informative $N(0, 10^2)$ priors on each parameter.
...
The regresison model is specified with `regression = ~(durnpso + prevsys + bsa + weight + psa)*.trt`, which will include the main (prognostic) effects of each covariate as well as interactions with treatment.
...
All of the above estimates (relative effects, predictions, rankings) can also be produced for a specific target population or populations by providing the `newdata` arguments to each function (and a `baseline` distribution for `predict()`).
If IPD are available for the target population(s), `newdata` is simply a data frame of the IPD.
If AgD are available for the target population(s), `newdata` must be a data frame with added integration points, created using the `add_integration()` function.
For example, suppose we wanted estimates for a population with the following aggregate characteristics:
```{r}
new_agd <- tibble(
  bsa_mean = 0.6,
  bsa_sd = 0.3,
...
We use the same weighted mean correlation matrix computed earlier from the IPD in the network, which is stored in the network object as `int_cor`.
```{r}
new_agd <- add_integration(new_agd,
  durnpso = distr(qgamma, mean = durnpso_mean, sd = durnpso_sd),
  prevsys = distr(qbern, prob = prevsys),
  bsa = distr(qlogitnorm, mean = bsa_mean, sd = bsa_sd),
  weight = distr(qgamma, mean = weight_mean, sd = weight_sd),
...
Population-adjusted relative effects for this target population are then given by
```{r pso_releff_FE_target}
(pso_releff_FE_new <- relative_effects(pso_fit_FE,
                                       newdata = new_agd))
plot(pso_releff_FE_new, ref_line = 0)
```

Predicted probabilities of achieving PASI 75 in this target population, given a $N(-1.75, 0.08^2)$ distribution on the baseline probability of response on Placebo (at the reference levels of the covariates), are given by
```{r pso_pred_FE_target}
