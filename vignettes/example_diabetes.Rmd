...
The `baseline` argument takes a `distr()` distribution object with which we specify the corresponding Normal distribution on the baseline cloglog probability, and we set `trt_ref = "Diuretic"` to indicate that the baseline distribution corresponds to "Diuretic" rather than the network reference "Beta Blocker".
...
```{r diabetes_tests, include=FALSE, eval=testthat::is_testing()}
