...
We demonstrate the model selection strategy proposed by Perren et al. to determine the most suitable class effects model. 
...
We follow the model selection stategey proposed by Perren et.al  

<img src="class_effects_flow_chart-1.png" width="600px" style="display: block; margin: auto;" />
...
```{r}
df <- do.call(rbind, sa_dic_RE$pointwise) %>% as.data.frame()
...
We can see from investigating the data that the two outlier studies are ALDEN2011 and EMMELKAMP2006 with their large `resdev` values in the no class model. 
...
Although both models yield similar residual deviance (162.6 vs. 163) and between‐study heterogeneity, the exchangeable class model reduces model complexity—reflected by a lower effective number of parameters (87.9 vs. 94.8) and a lower DIC (250.9 vs. 257.4). A deviance–deviance plot confirms that all data points fit equally well. Overall, these findings support using the class model for the social anxiety data.
...
Both random‐effects models showed substantially better fit than the fixed‐effect model (posterior mean residual deviance of 163 and 157.8 versus 284.8, and DIC values of 251 and 250.9 versus 319.1). Although the common class model achieved a slightly better absolute fit, it had higher between‐study heterogeneity (0.25 vs. 0.20) than the exchangeable class model.
